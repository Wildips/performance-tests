...
class Fake:
    ...
    def enum(self, value: type[TEnum]) -> TEnum:
        """
        Выбираем случайное значение из enum-типа.

        :param value: Enum-класс для генерации значения.
        :return: Случайное значение из перечня
        """
    ...
    def category(self) -> str:
        """
        Генерирует случайную категорию покупки из предопределенного списка.

        Используется для имитации типов расходов в системах, моделирующих
        пользовательские транзакции или поведение при оплате товаров или услуг.
...
# fake_us = Fake(faker=Faker("us_EN"))
# fake_en = Fake(faker=Faker("en_EN"))
