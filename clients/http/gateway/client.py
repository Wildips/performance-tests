...
def build_gateway_locust_http_client(environment: Environment) -> Client:
    ...
    (время ответа, длину ответа и т.д.) и отправляет их в Locust через `environment.evet.request`.

    Таким образом, данные клиент автоматически репортит статистику в Locust
