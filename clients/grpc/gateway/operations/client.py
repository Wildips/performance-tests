...
class OperationsGatewayGRPCClient(GRPCClient):
    ...
    def get_operations_api(self, request: GetOperationsRequest) -> GetOperationsResponse:
        ...
        :return: Ответ от сервиса с данными счета.
        """
    ...
    def make_transfer_operation_api(self, request: MakeTransferOperationRequest) -> MakeTransferOperationResponse:
        ...
        :param request: gRPC-запрос с данными операции перевода средств.
        :return: Ответ от сервиса с данными операции перевода средств.
        """
    ...
    def make_purchase_operation_api(self, request: MakePurchaseOperationRequest) -> MakePurchaseOperationResponse:
        ...
        :param request: gRPC-запрос с данными операции списания средств.
        :return: Ответ от сервиса с данными операции списания средств.
        """
    ...
    def make_bill_payment_operation_api(
        ...
        :param request: gRPC-запрос с данными операции оплаты счета.
    ...
    def make_cash_withdrawal_operation_api(
        ...
        :param request: gRPC-запрос с данными операции снятия средств.
        :return: Ответ от сервиса с данными операции снятия средств.
        """
        return self.stub.MakeCashWithdrawalOperation(request)

    def get_operations(self, account_id: str) -> GetOperationsResponse:
        """
        Получение данных по счету по его ID.

        :param account_id: Идентификатор счета.
        :return: Ответ с информацией об операциях.
        """
    ...
    def get_operations_summary(self, account_id: str) -> GetOperationsSummaryResponse:
        """
        Получение суммарных данных операций по счету по его ID.

        :param account_id: Идентификатор счета.
        :return: Ответ с информацией об суммарных операциях по счету.
        """
    ...
    def get_operation_receipt(self, operation_id: str) -> GetOperationReceiptResponse:
        """
        Получение чека операции по его ID.
    ...
    def get_operation(self, operation_id: str) -> GetOperationResponse:
        """
        Получение данных операции по ее ID.
    ...
    def make_cash_withdrawal_operation(self, card_id: str,
        ...
        """
        Создание операции выдачи средств.
...
def build_operations_gateway_locust_grpc_client(environment: Environment) -> OperationsGatewayGRPCClient:
    """
    Функция создает экземпляр OperationsGatewayGRPCClient адаптированный под Locust.

    Клиент автоматически собирает метрики и передает их в Locust через интерцептор.
    Используется исключительно в нагрузочных тестах.

    :param environment: объект окружения Locust.
    :return:экземпляр OperationsGatewayGRPCClient с интерцептором сбора метрик.
    """
